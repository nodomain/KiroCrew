...
async def _handle_discover(request: web.Request) -> web.Response:
    ...
    if kind == "repo":
        ...
        if rc != 0 or not await asyncio.to_thread(target.exists):
            # A failed or timed-out clone can leave a partial repo behind; remove
            # it now instead of waiting for the TTL sweep, so repeated failures
            # cannot accumulate and exhaust disk (ENOSPC). This does not change
            # the success path — a good clone is kept and rendered as before.
...
async def _handle_render(request: web.Request) -> web.Response:
    ...
    # _uploads_dir() itself mkdirs the base dir, so build the render dir entirely
    # off the event loop (base mkdir + the per-render mkdir).
